...
class Solution:
    def floodFill(self, image, sr, sc, newColor):
        ...
        if image[sr][sc]==newColor:
            ...
            self.subfloodFill(image, sr, sc, newColor,preColor)
            return image
    def subfloodFill(self, image, sr, sc, newColor,preColor):
        image[sr][sc]=newColor
        if sr-1>=0 and image[sr-1][sc]==preColor:
            self.subfloodFill(image, sr-1, sc, newColor,preColor)
        if sc-1>=0 and image[sr][sc-1]==preColor:
            self.subfloodFill(image, sr, sc-1, newColor,preColor)
