...
of the source code. This represents the result of splitting the original source code string by the newline character \n.
...
There will be no control characters, single quote, or double quote characters. For example, source = "string 
s = "/* Not a comment. */";" will not be a test case. (Also, nothing else such as defines or macros will interfere 
with the comments.)
...
After deletion, the implicit newline characters are deleted, leaving the string "ab", which when delimited by newline 
        ...
        while k<len_source:
            ...
            while i<len(source[k]):
                if source[k][i:i+2]=='//':
                    ...
                    if '*/' in source[k][i+2:]:
                        m=2
                        while source[k][i+m:i+m+2]!='*/':
                            m+=1
                        source[k]=source[k][:i]+source[k][i+m+2:]
                    else:
                        source[k]=source[k][:i]
                        while '*/' not in source[k+1]:
                            source.pop(k+1)
                            len_source-=1
                        m=0
                        while source[k+1][m:m+2]!='*/':
                            m+=1
                        source[k+1]=source[k+1][m+2:]
                        source[k]=source[k]+source[k+1]
                        source.pop(k+1)
                        len_source-=1
