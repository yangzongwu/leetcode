...
class Solution(object):
    def findDuplicate(self, paths):
        ...
        for path in paths:
            tmp=path.split(' ')
            for subpath in tmp[1:]:
                subtmp=subpath.split('(')
                context=subtmp[-1][:-1]
                if context not in dict_path:
                    dict_path[context]=[tmp[0]+'/'+subtmp[0]]
                else:
                    dict_path[context].append(tmp[0]+'/'+subtmp[0])
        rep=[]
        for keys in dict_path:
            if len(dict_path[keys])>1:
                rep.append(dict_path[keys])
        return rep
